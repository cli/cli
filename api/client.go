...
// NewClientFromHTTPWithAPIHost builds a Client that honors a per-host api_host
// override. apiHostFor maps a canonical hostname to its override host, returning
// "" when no override is configured, and tokenFor resolves the token for the
// canonical hostname. When an override applies, requests are routed to it while
// the token stays selected for the canonical host. This is a scoped prototype of
// the api_host routing plan.
func NewClientFromHTTPWithAPIHost(httpClient *http.Client, apiHostFor func(string) string, tokenFor func(string) string) *Client {
	return &Client{http: httpClient, apiHostFor: apiHostFor, tokenFor: tokenFor}
}

type Client struct {
	http       *http.Client
	apiHostFor func(string) string
	tokenFor   func(string) string
}
...
func (c Client) clientOptions(hostname string) ghAPI.ClientOptions {
	...
	// to that host and let go-gh attach the token. The token is selected for the
	// canonical hostname and go-gh's guard is widened to the override, so the
	// token stays bound to hostname even though the request is addressed to the
	// override endpoint.
	if c.apiHostFor != nil {
		if apiHost := c.apiHostFor(hostname); apiHost != "" {
