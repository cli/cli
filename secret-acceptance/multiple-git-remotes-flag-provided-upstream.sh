...
$_gh secret set ACCEPTANCE_KEY --body ACCEPTANCE_VAL --repo "$_upstream"

if [[ -z $($_gh secret list --repo "$_upstream" | grep ACCEPTANCE_KEY) ]]; then
    echo 'FAIL: expected secret list to contain ACCEPTANCE_KEY'
    exit 1
fi

$_gh secret delete ACCEPTANCE_KEY --repo "$_upstream"

if [[ -n $($_gh secret list --repo "$_upstream" | grep ACCEPTANCE_KEY) ]]; then
