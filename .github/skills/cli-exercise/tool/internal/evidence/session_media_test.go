	...
	t.Run("formats and selected capabilities", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				rendered, timeline, err := assembleSession(t.Context(), destination, receipt, sessionMediaTestChapters(), tc.formats, false)
				require.ErrorContains(t, err, tc.err)
...
func sessionMediaNativeCase(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			rendered, timeline, err := assembleSession(ctx, directory, receipt, chapters, formats, tc.progress)
			require.NoError(t, err)
	...
	t.Run("decoded metadata mismatch is a failure", func(t *testing.T) {
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				rendered, timeline, err := assembleSession(ctx, directory, receipt, input, []string{"mp4"}, false)
				require.ErrorContains(t, err, tc.err)
				require.Empty(t, rendered)
				require.Nil(t, timeline)
				require.NoFileExists(t, filepath.Join(directory, "session.mp4"))
	...
	t.Run("existing output is not overwritten", func(t *testing.T) {
		...
		rendered, timeline, err := assembleSession(ctx, directory, receipt, chapters, []string{"mp4"}, false)
	...
	t.Run("cancelled assembly is not a partial success", func(t *testing.T) {
		...
		stop()
		directory := filepath.Join(t.TempDir(), "output")
		rendered, timeline, err := assembleSession(cancelled, directory, receipt, chapters, []string{"mp4"}, false)
