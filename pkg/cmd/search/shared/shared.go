...
func displayIssueResults(io *iostreams.IOStreams, et EntityType, results search.IssuesResult) error {
	...
	for _, issue := range results.Items {
		...
		if issue.IsPullRequest() {
			tp.AddField(issueNum, nil, cs.ColorFromString(colorForPRState(issue)))
		} else {
			tp.AddField(issueNum, nil, cs.ColorFromString(colorForIssueState(issue)))
		}
		if !tp.IsTTY() {
...
func colorForIssueState(issue search.Issue) string {
	state := issue.State
	switch state {
	case "open":
		return "green"
...
func colorForPRState(pr search.Issue) string {
	state := pr.State
	switch state {
	case "open":
		return "green"
