...
func TestPrClose_alreadyClosedWithComment(t *testing.T) {
	http := &httpmock.Registry{}
	defer http.Verify(t)
	...
	http.Register(
		...
		{ "data": { "addComment": { "commentEdge": { "node": {
			...
			func(inputs map[string]any) {
				...
				assert.Equal(t, "cleanup notice", inputs["body"])
			}),
	)

	output, err := runCommand(http, true, "96 --comment 'cleanup notice'")
	assert.NoError(t, err)
	assert.Equal(t, "", output.String())
	assert.Equal(t, "! Pull request OWNER/REPO#96 (The title of the PR) is already closed\n", output.Stderr())
}
