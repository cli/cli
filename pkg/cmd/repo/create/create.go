...
func createFromScratch(opts *CreateOptions) error {
	...
	if opts.Interactive {
		...
		confirmed, err := opts.Prompter.Confirm(fmt.Sprintf(`This will create "%s" as a %s repository on %s. Continue?`, targetRepo, strings.ToLower(opts.Visibility), host), true)
		if err != nil {
			return err
		} else if !confirmed {
