...
/*
If you're reading this comment by necessesity, I'm sorry and if you're reading it for fun, you're welcome, you weirdo.
...
Here's a summary:
	...
	"a😅😅" takes 9 `bytes`
	In Go `len("a😅😅")` is 9 because the `len` builtin counts `bytes`, "a" = 1 byte and "😅" = 4 bytes
	In Go `len([]rune("a😅😅"))` is 3 because each `rune` is 4 `bytes` so each character fits within a `rune`
	In C# `"a😅😅".Length` is 5 because each `Char` is 2 bytes, so each "😅" requires 2 `bytes`.
