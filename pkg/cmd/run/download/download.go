...
func NewCmdDownload(f *cmdutil.Factory, runF func(*DownloadOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Doc(`
			...
			the artifact name. If only a single artifact is specified and '--pattern' is not specified, it will be extracted into
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) > 0 {
				opts.RunID = args[0]
			} else if (len(opts.Names) == 0 && len(opts.FilePatterns) == 0) && opts.IO.CanPrompt() {
				opts.DoPrompt = true
			}
...
func matchAnyName(patterns []string, name string) bool {
