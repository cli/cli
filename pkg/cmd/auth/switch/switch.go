...
func switchRun(opts *SwitchOptions) error {
	...
	if len(githubUsers) == 0 {
		return fmt.Errorf("not logged in to any GitHub hosts")
	}

	if len(githubUsers) == 1 {
		return fmt.Errorf("you are only logged into one account on %s: %s", ghinstance.Default(), githubUsers[0])
	}
