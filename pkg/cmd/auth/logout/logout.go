...
func NewCmdLogout(f *cmdutil.Factory, runF func(*LogoutOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Doc(`
			...
			For GitHub.com and GitHub Enterprise Cloud accounts, this command revokes
			OAuth tokens generated by the GitHub CLI. It also removes the stored
			authentication configuration and credentials stored by gh in the system
			credential store.

			Personal access tokens are removed from local storage only.
		`),
		Example: heredoc.Doc(`
...
func logoutRun(opts *LogoutOptions) error {
	...
	appendOAuthToken := func(token string) {
		if strings.HasPrefix(token, string(gh.TokenTypeOAuth)) && !slices.Contains(oauthTokens, token) {
			oauthTokens = append(oauthTokens, token)
		}
	}
	if token, _, err := authCfg.TokenForUser(hostname, username); err == nil {
		appendOAuthToken(token)
	}
	if preLogoutActiveUser == username {
	...
	if err := credentialUpdater.Reject(hostname, username); err != nil {
