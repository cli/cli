...
// num set to -1 indicates all posible values.
func (a *API) ListCodespaces(ctx context.Context, num int) (codespaces []*Codespace, err error) {
	var per_page int
	if num < 100 && num > 0 {
		per_page = num
	} else {
	...
	for page := 1; ; page++ {
		...
		if num > 0 && num <= response.TotalCount {
			break
		}
	}
