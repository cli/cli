...
func (a *App) Create(ctx context.Context, opts CreateOptions) error {
	...
	repo, err := a.getRepoName(opts.Repo)
	if err != nil {
		return fmt.Errorf("error getting repository name: %w", err)
	}
	branch, err := a.getBranchName(opts.Branch)
	if err != nil {
	...
	machine, err := a.getMachineName(ctx, repository.ID, opts.Machine, branch, locationResult.Location)
	if err != nil {
	...
	if opts.ShowStatus {
		if err := a.showCreateStatus(ctx, userResult.User, codespace); err != nil {
			return fmt.Errorf("show status: %w", err)
		}
	}

	a.logger.Print("Codespace created.")
	a.Println(codespace.Name)

	return nil
}

// showCreateStatus polls the codespace for a list of post create states and their status.
// It will keep polling until all states have finished. Once all states have finished,
// we poll once more to check if any new states have been introduced and stop polling otherwise.
func (a *App) showCreateStatus(ctx context.Context, user *api.User, codespace *api.Codespace) error {
	var (
		lastState      postCreateState
		breakNextState bool
	)
	...
	poller := func(states []postCreateState) {
		var inProgress bool
		for _, state := range states {
			...
			if state.Name != lastState.Name {
				...
				if state.Status == postCreateStateRunning {
					inProgress = true
					lastState = state
					a.Print("...")
					break
				...
				finishedStates[state.Name] = true
				a.Println(state.Status)
				lastState = postCreateState{} // reset the value
			}
		}
	...
	err := a.pollPostCreateStates(ctx, codespace, poller)
	if err != nil {
...
func (a *App) getRepoName(repo string) (string, error) {
	if repo != "" {
		return repo, nil
	}
...
func (a *App) getBranchName(branch string) (string, error) {
	...
	err := a.ask(branchSurvey, &branch)
	return branch, err
}

// getMachineName prompts the user to select the machine type, or validates the machine if non-empty.
func (a *App) getMachineName(ctx context.Context, repoID int, machine, branch, location string) (string, error) {
	machines, err := a.apiClient.GetCodespacesMachines(ctx, repoID, branch, location)
	if err != nil {
	...
	if err := a.ask(machineSurvey, &machineAnswers); err != nil {
